...
router.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    const { email, name, orgName } = (req.body ?? {}) as { email?: string; name?: string; orgName?: string };
    const e = (email || '').toString().trim().toLowerCase();
    if (!/.+@.+\..+/.test(e)) return res.status(400).json({ error: 'invalid email' });

    let org = await withDbTimeout(prisma.org.findFirst());
    if (!org) org = await withDbTimeout(prisma.org.create({ data: { name: orgName || 'Default Org' } }));
