...
/* ========================
 * Create a bot version under a bot
...
router.post(
  ...
  async (req: AuthReq, res: Response) => {
    ...
    const { prompts, tools, temperature } = parsed.data as {
      prompts?: unknown;
      tools?: unknown;
      temperature?: number;
    };
...
/* ========================
 * Update bot version (lifecycle). Only forward transitions: draft->prepared->published->retired
 * PATCH /api/bot-versions/:versionId
 * ======================== */
router.patch(
  '/bot-versions/:versionId',
  authMiddleware,
  requireOrg(),
  requireAnyRole(['instructor', 'admin']),
  async (req: AuthReq, res: Response) => {
    ...
    const { status, prompts, tools, temperature } = parsed.data as {
      status?: 'draft' | 'prepared' | 'published' | 'retired';
      prompts?: unknown;
      tools?: unknown;
      temperature?: number;
    };
    ...
    const nextIdx = status ? order.indexOf(status) : curIdx;
    ...
    logger.info(
      { orgId, versionId: updated.id, status: updated.status, route: 'PATCH /api/bot-versions/:versionId' },
      'Bot version updated'
    );
...
/* ========================
 * Deploy a bot to a course (with a published version)
...
router.post(
  ...
  async (req: AuthReq, res: Response) => {
    ...
    if (versionId) {
      version = await prisma.botVersion.findUnique({
        where: { id: versionId },
    ...
    try {
      ...
      logger.info(
        { orgId, botId, courseId, versionId: version.id, route: 'POST /api/bots/:botId/deploy' },
        'Bot deployed to course'
      );
